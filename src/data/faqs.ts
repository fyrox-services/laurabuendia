import type { FAQ } from "@/types";

export const FAQ_HOME: FAQ[] = [
  {
    heading: "¿Qué pasa si un hermano no quiere vender la casa?",
    text: "Si uno de tus hermanos no quiere vender la casa, tendrás que intentar un acto de conciliación, vender tu parte de la herencia o iniciar una extinción de condominio.",
  },
  {
    heading: "¿Se puede vender una herencia si un heredero no quiere?",
    text: "No, para que un inmueble se pueda vender, todos los propietarios deben estar de acuerdo con esta decisión.",
  },
  {
    heading:
      "¿Cómo elegir la mejor solución si en una herencia una parte no quiere vender?",
    text: "Primero averigua cuánto vale tu parte de la herencia y luego compara la cifra respecto a lo que podrías obtener en una subasta pública, teniendo en cuenta los gastos del abogado.",
  },
  {
    heading: "No quiero vender mi parte de herencia ¿Qué puedo hacer?",
    text: "Puedes comprar las otras partes al resto de herederos para hacerte con el pleno dominio, si no cuentas con la capacidad económica, podrías llegar a un acuerdo con los demás propietarios para pagar un alquiler.",
  },
  {
    heading: "¿Se puede vender un terreno si un heredero no quiere?",
    text: "En el caso de un terreno también puede un heredero negarse a vender su parte, la diferencia es que se podría llegar a un acuerdo y dividir las tierras según la legislación.",
  },
];

export const FAQ_VALORACION_DANO_CORPORAL: FAQ[] = [
  {
    heading: "¿Qué es exactamente la valoración del daño corporal?",
    text: "Es un proceso médico-legal que permite cuantificar las lesiones, secuelas y perjuicios sufridos por una persona tras un accidente, negligencia médica o agresión. Su objetivo es establecer de forma objetiva el alcance del daño y su posible compensación económica.",
  },
  {
    heading: "¿Es lo mismo que un informe pericial?",
    text: "No exactamente. La valoración del daño corporal es una parte concreta de un informe pericial, centrada en cuantificar lesiones y secuelas. Un informe pericial puede incluir, además, el análisis de la actuación médica, la adecuación a la lex artis y otras cuestiones técnicas.",
  },
  {
    heading: "¿Quién puede realizar este tipo de valoración?",
    text: "Solo un médico con formación específica en valoración del daño corporal y experiencia pericial. Es importante que sea un profesional independiente, para asegurar la objetividad del informe.",
  },
  {
    heading: "¿Es válida esta valoración para un juicio?",
    text: "Sí. Si el informe está firmado por un médico perito cualificado, puede utilizarse en procesos judiciales o como base para reclamaciones extrajudiciales. En muchos casos, se acompaña también de la **ratificación en sede judicial** si el procedimiento lo requiere.",
  },
];

export const FAQ_INFORME_MEDICO_PERICIAL: FAQ[] = [
  {
    heading: "¿Tiene validez legal un informe pericial médico?",
    text: "Sí. Es un documento oficial que puede ser presentado ante un juez, una aseguradora o en cualquier procedimiento legal. Cuando está redactado por un perito cualificado, tiene un gran peso probatorio.",
  },
  {
    heading:
      "¿Puedo solicitar un informe aunque no tenga claro si ha habido negligencia?",
    text: "Por supuesto. En muchos casos, se empieza con un informe de viabilidad para determinar si hay base médica y legal suficiente para elaborar un informe pericial completo.",
  },
  {
    heading: "¿Quién suele encargar estos informes?",
    text: "Tanto pacientes particulares como abogados, aseguradoras o incluso instituciones públicas. También puede encargarse a título preventivo, antes de iniciar acciones legales.",
  },
  {
    heading: "¿Cuánto tarda en elaborarse?",
    text: "Depende de la complejidad del caso, la disponibilidad de documentación y si se requiere exploración física. En general, se entrega en un plazo de entre una y tres semanas.",
  },
];

export const FAQ_ACCIDENTES_DE_TRAFICO_Y_HOGAR: FAQ[] = [
  {
    heading:
      "¿Puedo solicitar un informe aunque ya haya sido atendido por la aseguradora?",
    text: "Sí. Puedes recurrir a un perito médico independiente en cualquier momento del proceso si consideras que tu valoración no ha sido justa o completa.",
  },
  {
    heading: "¿Qué baremo se utiliza para calcular la indemnización?",
    text: "Se aplica el Baremo de la Ley 35/2015, que establece los criterios para valorar días de perjuicio, secuelas y otros daños derivados del accidente.",
  },
  {
    heading: "¿Este informe es válido en un juicio?",
    text: "Sí. Está redactado para ser utilizado en procedimientos judiciales y puede ser ratificado por el propio perito si el caso lo requiere.",
  },
  {
    heading: "¿Quién puede contratar este servicio?",
    text: "Tanto personas afectadas directamente por el accidente como abogados que representen sus intereses o incluso compañías aseguradoras que necesiten una valoración imparcial.",
  },
];

export const FAQ_INFORMES_DE_VIABILIDAD: FAQ[] = [
  {
    heading: "¿Es obligatorio hacer este informe antes del informe pericial?",
    text: "No es obligatorio, pero sí muy recomendable. Permite evitar la elaboración de un informe pericial completo en casos que no tienen base suficiente.",
  },
  {
    heading: "¿Este informe puede presentarse en un juicio?",
    text: "No. El informe de viabilidad no está pensado para ser utilizado como prueba judicial, sino como herramienta interna de análisis. En caso de que el caso sea viable, se elaborará un **informe pericial médico** con validez legal.",
  },
  {
    heading: "¿Y si el informe dice que el caso no es viable?",
    text: "Se explicarán los motivos médicos por los que no se considera viable. Este resultado también puede ser útil para cerrar el asunto con mayor tranquilidad y sin iniciar un proceso legal innecesario.",
  },
];

export const FAQ_PERDIDA_DE_OPORTUNIDAD: FAQ[] = [
  {
    heading: "¿Se puede reclamar una indemnización por pérdida de oportunidad?",
    text: "Sí, si se demuestra que hubo un retraso evitable que privó al paciente de una opción razonable de mejora. La indemnización no cubre el daño completo, sino la **probabilidad frustrada** de haber tenido un mejor resultado.",
  },
  {
    heading: "¿Qué diferencia hay con una negligencia médica?",
    text: "La pérdida de oportunidad no exige que el daño haya sido causado directamente por la actuación médica, sino por la omisión o retraso que impidió que se actuara a tiempo. Es un concepto más matizado.",
  },
  {
    heading: "¿Cómo se demuestra?",
    text: "A través de un informe pericial elaborado por un médico experto en valoración del daño corporal y análisis clínico, que analice todo el recorrido asistencial y sus consecuencias.",
  },
];